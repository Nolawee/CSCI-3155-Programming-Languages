object Lab5 extends jsy.util.JsyApplication {
   ...
   * Collaborators: Camilla Lambrocco
   */
  ...
  def mapFirstWith[W, A](f: A => Option[DoWith[W, A]])(l: List[A]): DoWith[W, List[A]] = l match {
    ...
    case h :: t => f(h) match {
      case None => mapFirstWith[W, A](f)(t) map { x => h :: x }
  ...
  def step(e: Expr): DoWith[Mem, Expr] = {
    ...
    e match {
      ...
      case Assign(GetField(a @ A(_), f), v) if isValue(v) =>
        for (
          _ <- domodify {
            //make a map for all of the fields and invoke domodify in the memory 
            (m: Mem) =>
              {
                if (m.contains(a)) {
                  //if mem contains stored address of the field 
                  val obj = m(a)
                  //obj  = get the memory at that address for the object 
                  val newobj = obj match {
                    case Obj(fields) => Obj(fields + (f -> (v)))
                    //make a new obj with with the fields of new field f mapped to v, the value 
                    case _ => throw StuckError(e)
                    //if there isnt any fields, throw an error
                  }
                  m + (a -> newobj)
                  //add to memory m with address to object 
                } else m
                //otherwise we return memory 
              }
          }
        ) yield v
